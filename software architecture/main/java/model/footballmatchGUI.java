...
public class footballmatchGUI<Match> {
    private JFrame frame;
    private JTextField playerIdField;
    private JPasswordField passwordField;
    private JButton loginButton;
    private String loggedInplayerId; // To store the logged-in user's ID
    private JTable scoreTable;
    private JScrollPane scrollPane; // To make the table scrollable

    public footballmatchGUI() {
        frame = new JFrame("Football Match");
    ...
    private void placeComponents(JPanel panel, int startY) {
        ...
        playerIdLabel.setBounds(startX, startY, 80, 25);
        panel.add(playerIdLabel);

        playerIdField = new JTextField(20);
        playerIdField.setBounds(startX + 90, startY, 160, 25);
    ...
    private void performLogin(ActionEvent e) {
        ...
        Authentication auth = new Authentication(playerId,password);
        if (auth.login()) {
            loggedInplayerId = playerId;
            String userRole = auth.verifyCredentials();
            if ("Player".equals(userRole)) {
                Playerlist player = fetchPlayerDetails(loggedInplayerId);
                if (player != null) {
    ...
    private void showPlayerScreen(Playerlist player) {
        ...
        updateScoreTable(loggedInplayerId);
    ...
    private void displayPlayerDetails(JPanel playerPanel, Playerlist player) {
        int yPos = 320;
       ...
       // String totalScoreWithWeightingString = String.valueOf(player.getTotalScoreWithWeighting());
        ...
        String scoresString = Arrays.stream(player.getScoreArray())
    ...
    private List<String[]> readMatchesForUser(String userID) {
        ...
        String line;
        String csvFile = "C:\\Users\\DELL\\OneDrive\\Desktop\\software architecture\\teams excel.csv";
    ...
    private void addMatchOutcomesToPlayer(Playerlist player) {
        String csvFile = "C:\\Users\\DELL\\OneDrive\\Desktop\\software architecture\\teams excel.csv";
        try (BufferedReader br = new BufferedReader(new FileReader(csvFile))) {
            String line;
            while ((line = br.readLine()) != null) {
                String[] values = line.split(",");
                if (values[1].equals(player.getPlayerId())) {
                    int score = Integer.parseInt(values[2]);
                    Match match = new Match(values[0], player.getPlayerId(), score, values[3]);
    ...
    private Playerlist fetchPlayerDetails(String loggedInplayerId) {
        String filePath = "C:\\Users\\DELL\\OneDrive\\Desktop\\software architecture\\RunCompetitorexcel.csv";
        try (BufferedReader br = new BufferedReader(new FileReader(filePath))) {
            String line;
            while ((line = br.readLine()) != null) {
                String[] values = line.split(",");
                if (values[0].equals(playerIdField)) {
                    ...
                    // Debug statements
                    System.out.println("Full line for playerId " + playerIdField + ": " + line);
                    System.out.println("Raw score string for playerId " + playerIdField + ": " + Arrays.toString(Arrays.copyOfRange(values, 5, values.length)));
                    System.out.println("Scores for playerId " + playerIdField + ": " + Arrays.toString(scores));

                    return new match(loggedInplayerId, name, age, country, scores);
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private void showTournamentStaffScreen() {
        ...
        // Dropdowns for team 1 and team 2
        JComboBox<String> team1Dropdown = new JComboBox<>(getPlayerIDs());
        JComboBox<String> team2Dropdown = new JComboBox<>(getPlayerIDs());
        team1Dropdown.setBounds(10, 130, 100, 25);
        team2Dropdown.setBounds(120, 130, 100, 25);
        staffPanel.add(team1Dropdown);
        staffPanel.add(team2Dropdown);

        // Points input fields for team 1 and team 2
        JTextField score1Field = new JTextField();
        JTextField score2Field = new JTextField();
        score1Field.setBounds(230, 130, 50, 25);
        score2Field.setBounds(290, 130, 50, 25);
        staffPanel.add(score1Field);
        staffPanel.add(score2Field);

        // Dropdown for Result
        JComboBox<String> resultDropdown = new JComboBox<>(new String[] {"team 1 Win", "team 2 Win"});
        ...
        addMatchButton.addActionListener(e -> addMatch(team1Dropdown, team2Dropdown, score1Field,score2Field, resultDropdown));
        
        JButton deleteteamButton = new JButton("Delete team");
        deleteteamButton.setBounds(360, 300, 120, 25);
        staffPanel.add(deleteteamButton);
        deleteteamButton.addActionListener(e -> deletePlayer((String) playerSelectDropdown.getSelectedItem()));
    ...
    private void updateMatchCsv(String player1ID, String player2ID, int points1, int points2, String winnerID) {
        String filePath = "C:\\Users\\DELL\\OneDrive\\Desktop\\software architecture\\teams excel.csv";
    ...
    private void showSelectedPlayerDetails(String selectedPlayerID) {
        Playerlist selectedPlayer = fetchPlayerDetails(selectedPlayerID);
    ...
    private void addMatchDetailsTable(JPanel panel, Playerlist selectedPlayer, int yPos) {
        ...
        selectedPlayer.getMatchOutcomes().forEach((matchID, match) -> {
...
//SORTINGS:
    ...
    private List<Playerlist> getAllPlayers() {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(filePath))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                Playerclass player = new Playerclass(playerId, name, age, country, scores);
    ...
    private String generateDetailedReport() {
        List<Playerlist> players = getAllPlayers();
        StringBuilder report = new StringBuilder();

        // Adding a header for the report
        report.append("Player ID\tName\tAge\tCountry\tTotal Points\tTotal Matches\tWins\tAverage Points\n");

        for (Playerlist player : players) {
            Dimension totalMatches = player.getMatchOutcomes().size();
           ...
           // double averagePoints = totalMatches > 0 ? (double) totalPoints / totalMatches : 0.0;
            ...
            report.append(player.getPlayerId())
    ...
    private int countWins(Playerlist player) {
        return 0;
    }
    ...
    private String generateReportSortedByAverageScore() {
        ...
        // Data rows
        for (Playerlist player : players) {
            double averageScore = (double) player.getOverallScore();
            report.append(player.getPlayerId()).append("\t")
    ...
    private String generateReportSortedByTotalPoints() {
        ...
        players.sort((p1, p2) -> Integer.compare(p2.getOverallScore(), p1.getOverallScore()));
    ...
    private int countWins(Playerclass player) {
        int wins = 0;
    ...
    private String generateReportSortedByAmountOfMatches() {
        ...
        // Data rows
        for (Playerlist player : players) {
            Dimension numberOfMatches = player.getMatchOutcomes().size();
            report.append(player.getPlayerId()).append("\t")
                  .append(player.getName()).append("\t")
